...
namespace TeacherControl.DataEFCore.Extensors
{
    public static class AssignmentLinqExtensors
    {
        ...
        public static IQueryable<Assignment> GetByDatesRange(this IQueryable<Assignment> assignments, DateTime StartDate, DateTime EndDate)
        {
             ...
             */
            if (EndDate.Equals(DateTime.MinValue)) assignments = assignments.Where(a => DateTime.Compare(StartDate, a.StartDate) >= 0);
            else if (StartDate.Equals(DateTime.MinValue)) assignments = assignments.Where(a => DateTime.Compare(EndDate, a.EndDate) <= 0);
            else if (DateTime.Compare(StartDate, EndDate) < 0) assignments = assignments.Where(a => DateTime.Compare(StartDate, a.StartDate) >= 0 && DateTime.Compare(EndDate, a.EndDate) <= 0);
        ...
        public static IQueryable<Assignment> GetByPointsRange(this IQueryable<Assignment> assignments, double PointsFrom, double PointsEnd)
        {
            ...
            else assignments = assignments.Where(i => i.Points >= PointsFrom && i.Points <= PointsEnd);
